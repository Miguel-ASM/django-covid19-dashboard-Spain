...
# Make a plot showing the evolution in a region
def makeRegionGrowthPlot(regionName):
    # Define basic layout of the plot
    fig_layout = Layout(
        showlegend = False,
        xaxis = {'showgrid': False, 'fixedrange':True},
    ...
    # data for the figure
    df = data.communities_data_frames_dict.get(regionName,pd.DataFrame())
    if df.empty :
        return {'name':regionName,'plot_div':'Not a valid region'}
    ...
    return {'name':regionName,'plot_div':plot_div}

# return growth plots for all the regions
def returnRegionsPlots():
    return [
        makeRegionGrowthPlot(regionName) for regionName in sorted(data.communities_data_frames_dict.keys())
    ]
